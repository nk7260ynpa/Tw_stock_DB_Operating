...
class TestCTEENewsAPI(unittest.TestCase):
    """測試 CTEE 新聞 API 端點。"""
    ...
    def test_list_uploaded(self, mock_router_cls):
        ...
        res = self.client.get("/api/ctee-news/uploaded")
    ...
    def test_get_schedule(self):
        """測試取得 CTEE 新聞排程設定。"""
        res = self.client.get("/api/ctee-news/schedule")
...
class TestLoadConfigWithCTEE(unittest.TestCase):
    """測試 load_config 向後相容（CTEE）。"""

    @patch("web_server.CONFIG_PATH")
    def test_missing_ctee_schedule_uses_default(self, mock_path):
        """測試缺少 ctee_schedule 時使用預設值。"""
        import web_server
    ...
    def test_existing_ctee_schedule_preserved(self, mock_path):
        ...
        self.assertEqual(config["ctee_schedule"]["time"], "22:30")


class TestCTEENewsScheduled(unittest.TestCase):
    """測試 CTEE 新聞排程任務（回溯時數）。"""
    ...
    def test_scheduled_uses_48_hours(self, mock_queue):
        ...
        web_server.run_ctee_news_scheduled()

        mock_queue.enqueue.assert_called_once()
        func, params = mock_queue.enqueue.call_args.args[1:]
        # enqueue(job_id, run_ctee_news_hours_job, (job_id, 48))
        self.assertEqual(func, web_server.run_ctee_news_hours_job)
