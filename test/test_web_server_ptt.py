...
class TestPTTNewsAPI(unittest.TestCase):
    """測試 PTT 新聞 API 端點。"""
    ...
    def test_create_upload_date_range(self, mock_queue):
        ...
        res = self.client.post(
            "/api/ptt-news/upload",
            json={"start_date": "2026-02-25", "end_date": "2026-02-28"},
        )
    ...
    def test_create_upload_invalid_date(self):
        ...
        res = self.client.post(
            "/api/ptt-news/upload",
            json={"start_date": "invalid", "end_date": "2026-02-28"},
        )
    ...
    def test_list_uploaded(self, mock_router_cls):
        ...
        res = self.client.get("/api/ptt-news/uploaded")
    ...
    def test_get_schedule(self):
        """測試取得 PTT 新聞排程設定。"""
        res = self.client.get("/api/ptt-news/schedule")
...
class TestLoadConfigWithPTT(unittest.TestCase):
    """測試 load_config 向後相容（PTT）。"""

    @patch("web_server.CONFIG_PATH")
    def test_missing_ptt_schedule_uses_default(self, mock_path):
        """測試缺少 ptt_schedule 時使用預設值。"""
        import web_server
        ...
        self.assertIn("ptt_schedule", config)
        self.assertEqual(config["ptt_schedule"]["time"], "21:20")

    @patch("web_server.CONFIG_PATH")
    def test_existing_ptt_schedule_preserved(self, mock_path):
        """測試已有 ptt_schedule 時保留原值。"""
        import web_server
...
class TestPTTNewsScheduled(unittest.TestCase):
    ...
    def test_scheduled_uses_48_hours(self, mock_queue):
        """排程於 21:20 抓取，回溯時數應為 48 小時以涵蓋昨日整天。"""
        import web_server
