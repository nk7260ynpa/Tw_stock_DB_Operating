...
class TestIndicesPriceAPI(unittest.TestCase):
    """測試股市指數價格 API 端點。"""
    ...
    def test_create_indices_price_upload_success(self, mock_queue):
        """測試成功建立股市指數價格上傳任務。"""
        mock_queue.enqueue.return_value = 0

        res = self.client.post(
            "/api/indices-price/upload",
            json={
    ...
    def test_create_indices_price_upload_invalid_date(self):
        """測試無效日期格式被拒絕。"""
        res = self.client.post(
            "/api/indices-price/upload",
            json={
    ...
    def test_create_indices_price_upload_end_before_start(self):
        """測試結束日期早於起始日期被拒絕。"""
        res = self.client.post(
            "/api/indices-price/upload",
            json={
    ...
    def test_list_uploaded_indices_price(self, mock_router_cls):
        ...
        res = self.client.get("/api/indices-price/uploaded")
    ...
    def test_get_indices_price_schedule(self):
        """測試取得股市指數價格每日排程設定。"""
        res = self.client.get("/api/indices-price/schedule")
    ...
    def test_update_indices_price_schedule_success(
    ...
    def test_update_indices_price_schedule_invalid_time(self):
        """測試無效時間被拒絕。"""
        res = self.client.put(
            "/api/indices-price/schedule",
            json={"time": "25:00"},
    ...
    def test_update_indices_price_schedule_empty_time(self):
        """測試空時間被拒絕。"""
        res = self.client.put(
            "/api/indices-price/schedule",
            json={"time": ""},
...
class TestLoadConfigIndicesPrice(unittest.TestCase):
    """測試 load_config 向後相容（股市指數價格）。"""

    @patch("web_server.CONFIG_PATH")
    def test_missing_indices_price_schedule_uses_default(self, mock_path):
        """測試缺少 indices_price_schedule 時使用預設值。"""
        import web_server
        ...
        self.assertEqual(
            config["indices_price_schedule"], {"time": "21:14"}
        )
