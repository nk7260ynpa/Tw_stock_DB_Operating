...
class TestMoneyUDNNewsAPI(unittest.TestCase):
    """測試 MoneyUDN 新聞 API 端點。"""
    ...
    def test_create_upload_success(self, mock_queue):
        """測試成功建立 MoneyUDN 新聞上傳任務。"""
        mock_queue.enqueue.return_value = 0

        res = self.client.post(
            "/api/moneyudn-news/upload",
            json={
    ...
    def test_list_uploaded(self, mock_router_cls):
        ...
        res = self.client.get("/api/moneyudn-news/uploaded")
    ...
    def test_get_schedule(self):
        """測試取得 MoneyUDN 新聞排程設定。"""
        res = self.client.get("/api/moneyudn-news/schedule")
    ...
    def test_update_schedule_success(
        ...
        res = self.client.put(
            "/api/moneyudn-news/schedule",
            json={"time": "23:00"},
        )

        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertEqual(data["time"], "23:00")
...
class TestLoadConfigWithMoneyUDN(unittest.TestCase):
    """測試 load_config 向後相容（MoneyUDN）。"""

    @patch("web_server.CONFIG_PATH")
    def test_missing_moneyudn_schedule_uses_default(self, mock_path):
        """測試缺少 moneyudn_schedule 時使用預設值。"""
        import web_server
        ...
        self.assertEqual(
            config["moneyudn_schedule"]["time"], "21:22"
        )

    @patch("web_server.CONFIG_PATH")
    def test_existing_moneyudn_schedule_preserved(self, mock_path):
        """測試已有 moneyudn_schedule 時保留原值。"""
        import web_server
        ...
        self.assertEqual(
            config["moneyudn_schedule"]["time"], "23:30"
        )


class TestMoneyUDNNewsScheduled(unittest.TestCase):
    """測試 MoneyUDN 新聞排程任務（回溯時數）。"""
    ...
    def test_scheduled_uses_48_hours(self, mock_queue):
        """排程於 21:22 抓取，回溯時數應為 48 小時以涵蓋昨日整天。"""
        import web_server

        self.assertEqual(web_server.NEWS_SCHEDULE_HOURS, 48)

        web_server.run_moneyudn_news_scheduled()

        mock_queue.enqueue.assert_called_once()
        func, params = mock_queue.enqueue.call_args.args[1:]
        self.assertEqual(func, web_server.run_moneyudn_news_hours_job)
