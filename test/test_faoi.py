...
from data_upload.faoi import Uploader, UploadType


class TestUploadType(unittest.TestCase):
    """測試 FAOI UploadType schema。"""
    ...
    def test_valid_data(self):
        ...
        data = UploadType(
            Date="2026-01-02",
            SecurityCode="2330",
            ForeignInvestorsTotalBuy=10000,
            ForeignInvestorsTotalSell=8000,
            ForeignInvestorsDifference=2000,
            ForeignDealersTotalBuy=500,
            ForeignDealersTotalSell=300,
            ForeignDealersDifference=200,
            SecuritiesInvestmentTotalBuy=1000,
            SecuritiesInvestmentTotalSell=800,
            SecuritiesInvestmentDifference=200,
            DealersDifference=100,
            DealersProprietaryTotalBuy=400,
            DealersProprietaryTotalSell=300,
            DealersProprietaryDifference=100,
            DealersHedgeTotalBuy=200,
            DealersHedgeTotalSell=200,
            DealersHedgeDifference=0,
            TotalDifference=2300,
        )

        self.assertEqual(data.SecurityCode, "2330")
        self.assertEqual(data.TotalDifference, 2300)
