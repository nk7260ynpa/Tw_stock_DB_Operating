...
從爬蟲服務取得國際股市指數價格資料（道瓊工業指數、納斯達克指數），
並上傳至 SPECIAL_INFO 資料庫的 IndicesPrice 表。
使用 REPLACE INTO 避免重複寫入，並記錄已上傳日期至 IndicesPriceUploaded 表。
"""
...
class IndicesPriceType(BaseModel):
    """股市指數價格資料 schema。

    欄位名稱對應 Tw_stock_DB 的 IndicesPrice 表結構。
    """
...
class IndicesPriceUploader:
    ...
    股市指數為非 24/7 市場（is_continuous_market=False），fallback 亦可能
    代表美股休市（如國慶／Juneteenth）。
    ...
    def check_uploaded(self, date):
        ...
        result = self.conn.execute(
            text(
                "SELECT COUNT(*) FROM IndicesPriceUploaded "
                "WHERE Date = :date"
            ),
            {"date": date},
        ).scalar()
    ...
    def crawl_data(self, date):
        """從爬蟲服務取得指定日期的股市指數價格資料。

        Args:
            date (str): 日期字串（YYYY-MM-DD）。

        Returns:
            pd.DataFrame: 股市指數價格 DataFrame。

        Raises:
            NetworkError: 網路連線失敗。
            CrawlError: 爬取失敗或資料格式異常。
        """
        url = f"http://{self.crawler_host}/indices_price"
        try:
            resp = requests.get(url, params={"date": date}, timeout=30)
            resp.raise_for_status()
        except (requests.ConnectionError, requests.Timeout) as e:
            raise NetworkError(
                f"股市指數價格爬蟲網路連線失敗（{date}）：{e}"
            ) from e
        except requests.RequestException as e:
            raise CrawlError(
                f"股市指數價格爬蟲呼叫失敗（{date}）：{e}"
            ) from e
        ...
        if "error" in result:
            ...
            if "無法取得任何" in error_msg:
                logger.info(
                    "股市指數價格 %s 爬蟲回報無資料（非交易日）：%s",
                    date, error_msg,
                )
                return pd.DataFrame()
            raise CrawlError(
                f"股市指數價格爬蟲回傳錯誤（{date}）：{error_msg}"
            )
    ...
    def check_schema(self, df):
        ...
        validated = [
            IndicesPriceType(**record).model_dump()
            for record in records
        ]
        return pd.DataFrame(validated)

    def _replace_into(self, df):
        """使用 REPLACE INTO 批次寫入 IndicesPrice 資料。
        ...
        sql = f"REPLACE INTO IndicesPrice ({col_str}) VALUES ({placeholder_str})"
    ...
    def _record_uploaded_date(self, date):
        """記錄已上傳日期至 IndicesPriceUploaded 表。
        ...
        self.conn.execute(
            text(
                "INSERT IGNORE INTO IndicesPriceUploaded (Date) "
                "VALUES (:date)"
            ),
            {"date": date},
        )
