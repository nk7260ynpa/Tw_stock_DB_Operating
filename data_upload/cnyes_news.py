...
從爬蟲服務取得 CNYES（鉅亨網）新聞資料，
將 metadata 寫入 MySQL NEWS.CNYES 表，
全文內容存為 md 檔至 /workspace/NewsContents/CNYES/。
"""
...
# 來源標籤，供 log 與錯誤訊息使用。
SOURCE_LABEL = "CNYES 新聞"

# 全文存放根目錄
NEWS_CONTENT_BASE = Path("/workspace/NewsContents/CNYES")


class CNYESNewsType(BaseModel):
    """CNYES 新聞 metadata schema。

    欄位名稱對應 NEWS.CNYES 表結構。
    注意：CNYES 沒有 SubHead 欄位（與 CTEE 不同）。
    """
...
class CNYESNewsUploader:
    """CNYES 新聞上傳器。

    1. 呼叫爬蟲 API 取得新聞資料
    2. metadata 寫入 MySQL NEWS.CNYES
    3. 全文存為 md 至 NewsContents/CNYES/YYYY-MM-DD/
    """
    ...
    def crawl_data(self, date):
        ...
        try:
            ...
            raise NetworkError(
                f"CNYES 新聞爬蟲網路連線失敗（{date}）：{e}"
            ) from e
        except Exception as e:
            logger.error("CNYES 新聞爬蟲呼叫失敗（%s）：%s", date, e)
            return pd.DataFrame()
        ...
        if not records:
            logger.info("CNYES 新聞 %s 無資料。", date)
            return pd.DataFrame()
    ...
    def crawl_data_by_hours(self, hours):
        ...
        try:
            ...
            raise NetworkError(
                f"CNYES 新聞爬蟲網路連線失敗（hours={hours}）：{e}"
            ) from e
        except Exception as e:
            logger.error(
                "CNYES 新聞爬蟲呼叫失敗（hours=%d）：%s", hours, e
            )
            return pd.DataFrame()
        ...
        if not records:
            logger.info("CNYES 新聞過去 %d 小時無資料。", hours)
            return pd.DataFrame()
    ...
    def get_existing_urls(self, date):
        ...
        rows = self.conn.execute(
            text("SELECT url FROM CNYES WHERE Date = :date"),
            {"date": date},
        ).fetchall()
    ...
    def filter_new_records(self, df, date):
        ...
        if skipped > 0:
            logger.info(
                "CNYES 新聞 %s 跳過 %d 筆已存在記錄。", date, skipped
            )
        return new_df
    ...
    def check_schema(self, df):
        ...
        for record in records:
            ...
            validated.append(CNYESNewsType(**meta).model_dump())
    ...
    def save_contents(self, df, date):
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                logger.error(
                    "CNYES 新聞全文儲存失敗（%s）：%s", file_path, e
                )

        return saved

    def upload_metadata(self, df):
        """上傳 metadata 至 NEWS.CNYES 資料表。
    ...
    def upload_by_hours(self, hours):
        ...
        logger.info(
            "CNYES 新聞（hours=%d）已上傳 %d 筆 metadata，"
            "儲存 %d 個全文檔案，涵蓋日期：%s。",
    ...
    def record_uploaded_date(self, date):
        ...
        try:
            self.conn.execute(
                text(
                    "INSERT IGNORE INTO CNYESUploaded (Date) VALUES (:date)"
                ),
                {"date": date},
            )
            self.conn.commit()
        except Exception as e:
            logger.error("記錄 CNYES 已上傳日期失敗（%s）：%s", date, e)
