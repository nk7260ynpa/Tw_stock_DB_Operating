...
- **SPECIAL_INFO 缺漏自我修復**：每日（預設 21:27，排在各商品 21:06~21:14 抓取之後補齊）對原油／黃金／比特幣／匯率／股市指數掃描近 30 天缺漏並自動補回，以「問爬蟲」為交易日／休市的唯一真相來源（回傳該日自身 K 棒即補上，只回更早日期即視為非交易日）。掃描窗遠大於各商品排程的「過去 7 天」回補窗，足以自癒管線停擺數週造成的缺漏。共用邏輯於 `data_upload/special_info_common.py`，一次性修復與孤兒帳本清理入口為 `backfill_special_info.py`
...
- **原油價格**：選擇日期範圍上傳國際原油價格（WTI/Brent），資料寫入 SPECIAL_INFO 資料庫，每日排程 21:06 自動抓取（21:06 台灣時間 = 13:06 UTC，前一美股交易日已收盤定案；當日美股尚未開盤，爬蟲會回退到最近交易日）
- **黃金價格**：選擇日期範圍上傳國際黃金期貨價格，資料寫入 SPECIAL_INFO 資料庫，每日排程 21:08 自動抓取
- **比特幣價格**：選擇日期範圍上傳比特幣價格，資料寫入 SPECIAL_INFO 資料庫，每日排程 21:10 自動抓取
- **匯率**：選擇日期範圍上傳匯率資料（USDTWD/JPYTWD），資料寫入 SPECIAL_INFO 資料庫，每日排程 21:12 自動抓取
- **股市指數**：選擇日期範圍上傳股市指數價格（道瓊工業指數/納斯達克指數），資料寫入 SPECIAL_INFO 資料庫，每日排程 21:14 自動抓取
...
| 21:06 | 原油 | SPECIAL_INFO 原油價格（過去 7 天回補） |
| 21:08 | 黃金 | SPECIAL_INFO 黃金價格（過去 7 天回補） |
| 21:10 | 比特幣 | SPECIAL_INFO 比特幣價格（過去 7 天回補） |
| 21:12 | 匯率 | SPECIAL_INFO 匯率（過去 7 天回補） |
| 21:14 | 股市指數 | SPECIAL_INFO 道瓊/納斯達克（過去 7 天回補） |
...
| 21:27 | SPECIAL_INFO 缺漏自我修復 | 排在各商品之後補齊近 30 天缺漏 |
...
- **YT 抓昨日**：21:24 執行時「當日」直播多半尚未結束或自動字幕尚未產生，故排程抓
  「昨日」已完成的直播影片，確保逐字稿已可取得。「昨日」以 `datetime.now()` 減一天
  計算，21:24 與原本的 07:54 同屬當日、不跨午夜，搬窗不會使日期偏移，與
  `Tw_stock_crawer` 的「YT 抓昨天」契約維持一致。
