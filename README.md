...
- **CTEE 新聞**：選擇日期範圍上傳工商時報新聞，metadata 寫入 MySQL，全文存為 txt 檔，每日排程（07:46）回溯過去 48 小時抓取，涵蓋昨日整天
- **CNYES 新聞**：選擇日期範圍上傳鉅亨網新聞，metadata 寫入 MySQL，全文存為 md 檔，每日排程（07:48）回溯過去 48 小時抓取，涵蓋昨日整天
- **PTT 新聞**：選擇日期範圍上傳 PTT 股版新聞，metadata 寫入 MySQL，全文存為 md 檔，每日排程（07:50）回溯過去 48 小時抓取，涵蓋昨日整天
- **MoneyUDN 新聞**：選擇日期範圍上傳經濟日報新聞，metadata 寫入 MySQL，全文存為 md 檔，每日排程（07:52）回溯過去 48 小時抓取，涵蓋昨日整天
- **公司產業對照**：一鍵從爬蟲取得最新 TWSE/TPEX 公司基本資料與產業對照表，寫入 TWSE 資料庫
- **YT 逐字稿**：選擇日期抓取「游庭皓的財經皓角」YouTube 直播影片逐字稿（透過 yt-dlp 下載自動字幕），metadata 寫入 MySQL，全文存為 md 檔，每日排程（07:54）自動抓取**昨日**已完成直播的逐字稿（早上執行時當日直播多半尚未結束或字幕未產生）
- **原油價格**：選擇日期範圍上傳國際原油價格（WTI/Brent），資料寫入 SPECIAL_INFO 資料庫，每日排程 07:36 自動抓取（美國市場收盤後台灣早上資料已可取得）
- **黃金價格**：選擇日期範圍上傳國際黃金期貨價格，資料寫入 SPECIAL_INFO 資料庫，每日排程 07:38 自動抓取
- **比特幣價格**：選擇日期範圍上傳比特幣價格，資料寫入 SPECIAL_INFO 資料庫，每日排程 07:40 自動抓取
- **匯率**：選擇日期範圍上傳匯率資料（USDTWD/JPYTWD），資料寫入 SPECIAL_INFO 資料庫，每日排程 07:42 自動抓取
- **股市指數**：選擇日期範圍上傳股市指數價格（道瓊工業指數/納斯達克指數），資料寫入 SPECIAL_INFO 資料庫，每日排程 07:44 自動抓取
...
所有「爬蟲抓取類」排程集中於 **07:30~08:00** 之間並彼此錯開，避免同時併發搶爬蟲
...
| 07:40 | 比特幣 | SPECIAL_INFO 比特幣價格（過去 7 天回補） |
| 07:42 | 匯率 | SPECIAL_INFO 匯率（過去 7 天回補） |
| 07:44 | 股市指數 | SPECIAL_INFO 道瓊/納斯達克（過去 7 天回補） |
| 07:46 | CTEE 新聞 | 回溯過去 48 小時 |
| 07:48 | CNYES 新聞 | 回溯過去 48 小時 |
| 07:50 | PTT 新聞 | 回溯過去 48 小時 |
| 07:52 | MoneyUDN 新聞 | 回溯過去 48 小時 |
| 07:54 | YT 逐字稿 | 抓**昨日**已完成直播的逐字稿 |
| 07:57 | SPECIAL_INFO 缺漏自我修復 | 排在各商品之後補齊近 30 天缺漏 |

不受此集中政策約束的維護型排程：**06:30** exhausted 隔日重排（排在早上抓取前，
把前一日放棄的任務重設為 pending 供當日重試）、重試佇列**每小時**檢查。

### 幾個正確性設計

- **新聞回溯 48 小時**：新聞改於早上抓取，回溯窗須涵蓋「昨日整天到現在」。設為
  48 小時（爬蟲支援 1~72），足以吸收「晚間排程→早晨排程」搬遷的一次性缺口，並可
  補回偶發漏抓一日；重複抓到的記錄由各上傳器以 URL 去重（同日已存在的 URL 會跳過），
  不會重複寫入。
...
免費機會。門檻要攔的是「來源擋人／全文頁改版」這類系統性異常（失敗率高到五分之一，
明天多半也修不好），零星幾篇交給隔日自然補抓即可。反之 `list_failed`／`deadline`
連「有哪些文章」都不知道，損失無上限且 CTEE 來源僅保留約 3 天，等不起。
